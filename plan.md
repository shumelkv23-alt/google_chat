...
## B3 — Загрузка резюме и подбор подходящих вакансий ✅ (реализовано; есть тех-долг по auth)

Обратное направление RAG: пользователь кидает резюме боту → бот ищет подходящие
открытые вакансии.

**Что сделано:**
- [x] Приём резюме в [interactions.py](app/api/interactions.py): файл (PDF/DOCX) **или**
      текст с префиксом «резюме:».
- [x] Извлечение текста из вложений ([attachments.py](app/services/attachments.py)):
      скачивание через Chat media API + парсинг `pypdf`/`python-docx`, детект сканов
      (пустой текст), лимит страниц/размера, потоковая загрузка, понятные подсказки
      юзеру по статусам (401/403 → права, 413 → размер).
- [x] Эмбеддинг резюме → cosine top-K по `vacancies.embedding`
      (status ≠ closed/filled, is_deleted=false) в [resume_match.py](app/services/resume_match.py).
- [x] LLM-ранжирование [resume_matcher.py](app/llm/resume_matcher.py): score 0-100 +
      объяснение «почему подходит / чего не хватает» по каждой вакансии.
- [x] Текстовый список матчей в ответ + подсказка про фичу в приветствии бота.
- [x] Тесты: [test_attachments.py](tests/test_attachments.py),
      [test_resume_match.py](tests/test_resume_match.py),
      [test_interactions_resume.py](tests/test_interactions_resume.py).

> ⚠️ **Тех-долг (на будущее): скачивание файлов сейчас работает только под user-OAuth.**
> Логика извлечения текста из PDF/DOCX готова и покрыта тестами, но само скачивание
> вложения ([attachments.py](app/services/attachments.py) `_download_media`) идёт под
> текущим user-OAuth ([google_oauth.py](app/services/google_oauth.py)). Этого хватает,
> только когда резюме присылает **сам** OAuth-пользователь; чужое вложение из DM другого
> юзера user-auth не отдаёт (401/403 → юзер видит аккуратную подсказку «пришли текстом»).
> **Чтобы файловая загрузка работала у всех — нужен service-account бота (app-auth) с
> Chat API** (scope `chat.bot`): завести SA, прокинуть его credentials в `_download_media`
> вместо `get_credentials()` — замена точечная. Текстовый путь («резюме:») от этого не
> зависит и работает для всех уже сейчас.
