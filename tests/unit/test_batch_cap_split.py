"""Юнит-тесты чистых помощников batch-конвейера v2 (без БД и сети).

_cap_by_tokens — токен-бюджет пачки с резкой по границе треда (B6):
тред атомарен, остаток уходит следующему тику.
_split_on_thread_boundary — бисекция ядовитой пачки (B10): режем пополам,
но не разрываем тред; если вся пачка — один тред, режем как есть.
"""
