"""Юнит-тесты согласованности маппингов полей вакансии.

Поле вакансии живёт сразу в трёх местах: колонка модели Vacancy, _FIELD_MAP
(extractor-ключ → имя колонки) и _REVERTABLE_FIELDS (что откатываем при удалении
источника). Рассинхрон тихий и неприятный: новое поле либо не доедет до БД, либо
не откатится при удалении сообщения. Эти тесты фиксируют инвариант — добавил
поле, но забыл маппинг → красный тест, а не молчаливая потеря данных.

Чистая логика: сверка структур в памяти, без БД/сети.
"""
